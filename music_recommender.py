...
class MusicRecommender:
    def __init__(self):
        ...
        try:
            ...
            if not client_id or not client_secret:
                print("⚠️  경고: Spotify API 키가 설정되지 않았습니다.")
                print("   음악 추천 기능을 사용하려면 .env 파일에 다음을 추가하세요:")
                print("   SPOTIFY_CLIENT_ID=your_client_id")
                print("   SPOTIFY_CLIENT_SECRET=your_client_secret")
                return  # self.sp는 None으로 유지
    ...
    def get_top_tracks_for_emotion(self, emotion, limit=50):
        ...
        """
        if not self.sp:
            print("⚠️  Spotify API가 연결되지 않았습니다.")
            return []
        ...
        try:
            ...
            print(f"🔍 '{emotion}' 감정 검색: 키워드 '{selected_keyword}'")
            
            # Spotify 검색 (재시도 로직 추가)
            max_retries = 3
            ...
            for item in results['tracks']['items']:
                ...
                # 필수 정보가 있는 트랙만 추가
                if track_info['name'] and track_info['url']:
    ...
    def recommend_music_by_emotions(self, emotions_dict, total_tracks=10):
        ...
        """
        if not self.sp:
            print("❌ Spotify API가 연결되지 않았습니다.")
            print("   음악 추천 기능을 사용할 수 없습니다.")
            return []
        ...
        for emotion, score in sorted_emotions:
            ...
            count = min(count, remaining_tracks)
        ...
        # ✅ 모든 감정이 5% 미만일 경우 처리
        if not emotion_track_counts:
            print("\n⚠️  모든 감정이 5% 미만입니다.")
            print("   최상위 감정으로 추천합니다.")
            top_emotion = sorted_emotions[0][0]
            return self.recommend_music(top_emotion, limit=total_tracks)
        
        # 남은 트랙은 최상위 감정에 할당
        if remaining_tracks > 0:
            top_emotion = list(emotion_track_counts.keys())[0]
        ...
        for emotion, count in emotion_track_counts.items():
            ...
            if top_tracks:
                ...
                print(f"⚠️  '{emotion}' 트랙을 찾을 수 없습니다.")
